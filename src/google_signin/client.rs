            ...
            loop {
                if let Ok(request) = reqwest::get(super::CERTS_URI).await {
                    ...
                    let max_age = if let Some(cache_control_value) = {
                        ...
                        if let Some(cache_control) = cache_control::CacheControl::from_value(cache_control_value) {
                    ...
                    if let Ok(text) = request.text().await {
                        let jwks: JwkSet = serde_json::from_str(&text[..])
                            .expect("Invalid certs URI.");

                        *decoding_keys.lock().unwrap() = Some(
                            jwks.keys.iter().map(|jwk| {
                                (
                                    match &jwk.algorithm {
                                    ...
                                    jwk.common.key_id.clone().unwrap_or_else(|| unreachable!("Key id must be present.")),
                                 )
